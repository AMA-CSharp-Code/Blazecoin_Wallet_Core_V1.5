...
| 2026-08-26 | Activation + Release (macOS) | **Phoenix-413 ACTIVATED on `macos-v1.5.0`: `PHOENIX_ACTIVATION_HEIGHT` = 4,194,000; `CLIENT_VERSION_IS_RELEASE` → true (`f017836`).** Network tip was 4,193,990 (both seed nodes' handshake) when H_A was chosen — the chain is difficulty-stranded and forks to per-block ASERT at 4,194,000. Universal (arm64 + x86_64) build compiled, 45/45 harness green on both slices, installed to `/Applications`. **GitHub release `v1.5.2-macos.1` published, marked Latest** — asset `Blazecoin-V1.5.2-macOS-universal.zip` (~50 MB), sha256 `b2d9686c…d008668`; notes carry the mandatory-update warning + new bootstrap peers `54.39.23.245:55414` / `51.210.47.141:55414` (the 2014 seeds freeze at the fork by design). |
...
- **`PHOENIX_ACTIVATION_HEIGHT` = `4194000` in this tree (ACTIVATED 2026-08-26 on the macOS branch).** H_A was set the same day the feature landed: the chain was difficulty-stranded with the network tip at 4,193,990, and Phoenix-413 is the recovery. The same H_A must ship in the V2 daemon and every lite-wallet head before the chain reaches block 4,194,000. The spec's checkpoint at/near H_A (§4) is still owed in a follow-up build once the fork block is buried.
