...
| 2026-09-14 (evening) | Releases, §12 | **V1.5.3 RELEASED** — `v1.5.3-windows` (tag on `dev` `0958152`; `Blazecoin-V1.5.3-windows-x64.zip` sha `3176d888…`, GUI + daemon rebuilt from `dev`, 1.5.2 bundle layout reused) and `v1.5.3-linux` (tag on `linux` `26cb93d`; `blazecoin-v1.5.3-linux-x86_64.tar.gz` sha `f90491a8…`, Ubuntu 22.04 WSL, `USE_UPNP=- CXXFLAGS=-std=gnu++14`, binaries stripped, Latest). macOS not cut (needs the operator's Mac; `v1.5.2-macos.1` stays current there). All four binaries verified to carry the `add35fb6…` anchor (`strings`); the §12 checkpoint item is closed. Correction: the `v1.5.2-linux` release body's "also includes the post-fork checkpoint" line was inaccurate — that tag predates `ba3da07`. Build finds: the WSL `makefile.unix` build must be given `CXXFLAGS=-std=gnu++14` explicitly (g++ 11 defaults to C++17 and rejects the 0.8-era `throw()` specifications); the Windows GUI must be built from `dev` (the `linux` branch carries Linux-only GUI commits — a `linux`-branch Windows build differs by 1 KB); the Windows binaries embed the stale `g55c5572` build id, as 1.5.2's did. |
| 2026-09-14 | §2.5 (new), §2.1 | **V1.5.3 (toward): checkpoint 4,194,000 baked into the binaries + sunset notice.** The Phoenix anchor checkpoint (`ba3da07`, 2026-08-26 evening) landed AFTER the three `v1.5.2-*` tags were cut, so no released binary carries it — this bump closes that gap. No consensus change. README gains the sunset notice: the post-quantum output type (`PQ_SIGNATURES.md`, V2 repo, 13 decisions ratified 2026-09-14) will not be ported to V1.5; V1.5 stops at that fork height. `CLIENT_VERSION_REVISION` → 3. |
...
### 2.5 V1.5.3 — checkpoint 4,194,000 in released binaries + sunset notice (2026-09-14)

Two things, neither of them consensus:

- **The Phoenix-413 anchor checkpoint ships.** `src/checkpoints.cpp` has carried `(4194000, add35fb6…)` since `ba3da07` (2026-08-26, "rides the next release"), but every `v1.5.2-*` tag points at an earlier commit, so the released 1.5.2 binaries validate the fork without the pin. 1.5.3 is that pin, released. Checkpoints are a sync-time guard, not a rule change: a 1.5.2 node and a 1.5.3 node accept exactly the same chain.
- **The sunset notice.** The chain's next consensus change is the post-quantum output type (`PQ_SIGNATURES.md` in the V2 repo — `P2PQH`, ML-DSA-44, a new opcode, a relaxed push limit and a new digest; all 13 design decisions ratified 2026-09-14, activation height not yet chosen, months of notice promised). It will **not** be implemented in this tree: teaching the 0.8.6.2 script engine a 5,000-byte push, opcode `0xba`, a BIP-143-shaped digest and an ML-DSA verifier is a rewrite, not a patch (`PQ_SIGNATURES.md` §7). At that height V1.5 nodes reject the first block carrying a post-quantum spend and freeze. Funds are never at risk — keys, UTXOs and the legacy transaction format are unchanged — and the migration path is the existing one: import the wallet into V2.
- **Version:** `CLIENT_VERSION_REVISION` 2 → 3 (`IS_RELEASE` stays `true`); the tree reports `v1.5.3.0` from this commit, the release is the tag.
...
- ✅ *(Resolved 2026-09-14: `v1.5.3-windows` / `v1.5.3-linux` released with the checkpoint — see the 2026-09-14 (evening) row; a macOS 1.5.3 still needs an on-Mac build.)* **The three `v1.5.2-*` tags do NOT contain the 4,194,000 checkpoint** that HEAD carries (`git grep 4194000 v1.5.2-windows -- src/checkpoints.cpp` is empty for `-windows`, `-linux` and `-macos.1`; found 2026-09-10). The §2.4 anchor-cache safety argument assumed it shipped. Decision owed: cut v1.5.3 with the checkpoint, or record that 1.5.2 relies on the anchor cache alone.
