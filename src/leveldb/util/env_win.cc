...
{
	Status sRet;
	DWORD attr = ::GetFileAttributesA(dirname.c_str());  // V1.5: Use ANSI version for char* strings
	...
	if (!sRet.ok()) return sRet;
	  ...
	  BOOL result = ::CreateDirectoryA(dirname.c_str(), NULL);  // V1.5: Use ANSI version for char* strings
      if (result == FALSE) {
